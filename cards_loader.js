    ...
    fetch("fetch_cards.php")
        ...
        .then(cards => {
            ...
            cards.slice(0, 3).forEach((card, index) => {
                const imgSrc = card.imagem ? `http://localhost${card.imagem.replace('..', '')}` : `http://localhost/static/placeholder.png`;
                const cardDiv = document.createElement("div");
                cardDiv.classList.add("card");
                cardDiv.innerHTML = `
                    <img src="${imgSrc}" alt="${card.nome_carta}" style="max-width: 100px;">
